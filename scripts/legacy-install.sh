...
auto_detect_cidr(){
  local local_ip subnet suggested_cidr
  ...
  log_info "自动检测到本机IP: ${GREEN}${local_ip}${NC}"
  log_info "自动使用扫描网段: ${GREEN}${suggested_cidr}${NC}"
  echo "$suggested_cidr"
}
...
def prewarm_neighbors(net: IPv4Network) -> None:
    """扫描前做一轮轻量预热，提升 ARP/邻居表命中率。"""
    try:
        hosts = [str(h) for h in islice(net.hosts(), min(CIDRFALLBACKLIMIT, 256))]
        # 分两段预热，避免一次性压满
    ...
    for attempt in range(max(1, SCAN_RETRIES)):
        try:
            ...
            if r2.status_code == 200:
                return True, realm
            raise RuntimeError(f"auth status {r2.status_code}")
        except Exception:
